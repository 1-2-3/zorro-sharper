...
export class NsFlipDirective {
  ...
  constructor(
    ...
    this._onResize$.pipe(throttleTime(2)).subscribe(() => {
      ...
      if (this._frontDiv && this._frontDiv.getBoundingClientRect) {
        let frontBound = this._frontDiv.getBoundingClientRect();
        let backBound = this._backDiv.getBoundingClientRect();
  ...
  ngAfterViewInit(): void {
    ...
    if (
      ...
      let trigger = this._frontDiv.querySelector("[nsFlipTrigger]");
    ...
    if (
      ...
      let trigger = this._backDiv.querySelector("[nsFlipTrigger]");
      this._renderer.listen(trigger, "click", e => {
        e.stopPropagation();
        this.flipCardH();
      });
  ...
  private _updateView() {
    let senceDiv = this._renderer.createElement("div"); // 最外层容器
    let cardDiv = this._renderer.createElement("div"); // 卡片容器
    let frontDiv = this._renderer.createElement("div"); // 卡片正面容器
    let backDiv = this._renderer.createElement("div"); // 卡片背面容器
    ...
    let frontView = this._viewContainer.createEmbeddedView(this._templateRef); // 正面内容
    let backView = this._viewContainer.createEmbeddedView(this._backTpl); // 背面内容
